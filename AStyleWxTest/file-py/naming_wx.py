...
__astylewx_src_dir = libastylewx.get_astylewx_directory() + "/src/"
if os.name == "nt":
    __cppcheck_exe = "C:/Program Files/Cppcheck/cppcheck.exe"
else:
    __cppcheck_exe = "cppcheck"

__astylewx_file_names = []
# can also check selected files in a list
#__astylewx_file_names = ["AStyleDlg.cpp", "EditorDlg.cpp"]

# Variables start with lower case,
# OR underscore plus lower case for constructor variables,
# OR all caps with underline allowed for constant expressions.
# Others are are allowed exceptions, DLL entry-point and others.
RE_VARNAME = "[a-z].*|_[a-z].*|^[A-Z]+(?:_[A-Z]+)*$|BOMSize|ErrorHandler|MemoryAlloc"
# Functions names start with upper case.
RE_FUNCTIONNAME = "[A-Z].*"
...
def create_dump_file(filename):
    ...
    """
    filepath = __astylewx_src_dir + filename
    outpath  = __astylewx_src_dir + "dump.txt"
    ...
    try:
        ...
        libastylewx.system_exit("Cannot find cppcheck executable")
    outfile.close()
    os.remove(outpath)
...
def get_astyle_file_names():
    ...
    """
    file_names = []
    filepaths = glob.glob(__astylewx_src_dir + "*.cpp")
    # remove the path
...
def process_dump_file(file):
    ...
    function_names = 0
    class_names    = 0
    filepath = __astylewx_src_dir + file
        ...
        if RE_CLASSNAME:
            for scope in cfg.scopes:
                if scope.type in ('Class', 'Struct', 'Enum'):
                    ...
                    # verify scope types
                    if scope.type in ('Global', 'Namespace', 'Function', 'Unconditional'):
                        continue
